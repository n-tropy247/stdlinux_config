...
() {
	...
	SEGMENT_SEPARATOR_LEFT_THIN=$'\ue0b1'
	SEGMENT_SEPARATOR_RIGHT=$'\ue0b2'
	SEGMENT_SEPARATOR_RIGHT_THIN=$'\ue0b3'
}
...
preprompt_segment() {
	...
	if [[ $CURRENT_BG == 'NONE' ]]; then
		CURRENT_SEGMENT="%{$bold%K{$bg}%F{$fg}%} "
	else
		if [[ $bg != $CURRENT_BG ]]; then
			if [[ $align == "right" ]]; then
				CURRENT_SEGMENT=" %{%b%K{$CURRENT_BG}%F{$bg}%}$SEGMENT_SEPARATOR_RIGHT"
			else
				CURRENT_SEGMENT=" %{%b%F{$CURRENT_BG}%K{$bg}%}$SEGMENT_SEPARATOR_LEFT"
			fi
			CURRENT_SEGMENT+="%{$bold%K{$bg}%F{$fg}%} "
		elif [[ $bg == $CURRENT_BG ]]; then
			if [[ $align == "right" ]]; then
				CURRENT_SEGMENT=" %{%b%K{$CURRENT_BG}%F{$bg}%}$SEGMENT_SEPARATOR_RIGHT_THIN"
			else
				CURRENT_SEGMENT=" %{%b%F{$fg}%K{$bg}%}$SEGMENT_SEPARATOR_LEFT_THIN"
			fi
			CURRENT_SEGMENT+="%{$bold%K{$bg}%F{$fg}%} "
		fi
	fi
	...
	[[ -n $1 ]] && CURRENT_SEGMENT+=$1
}

# End the prompt, closing any open segments
preprompt_end() {
	CURRENT_SEGMENT=" %{%b%k%F{$CURRENT_BG}%}$SEGMENT_SEPARATOR_LEFT%{%k%f%b%}"
	CURRENT_BG='NONE'
}
...
prompt_pure_preprompt_render() {
	...
	if [[ -n $prompt_pure_vcs_info[branch] ]]; then
		preprompt_segment '${prompt_pure_vcs_info[branch]}${prompt_pure_git_dirty}' ${git_color} default regular left
		preprompt_parts+=$CURRENT_SEGMENT
	fi
	# Git pull/push arrows.
	if [[ -n $prompt_pure_git_arrows ]]; then
		preprompt_segment '${prompt_pure_git_arrows}' cyan default regular left
		preprompt_parts+=$CURRENT_SEGMENT
	fi
	...
	if [[ -n $prompt_pure_cmd_exec_time ]]; then
		preprompt_segment '${prompt_pure_cmd_exec_time}' black yellow bold left
		preprompt_parts+=$CURRENT_SEGMENT
	fi
	...
	# local right=$(preprompt_segment '%* ' black default bold right)
	...
	# local middle=$(preprompt_segment ${(r:$((COLUMNS - left_length - right_length - 2)):)} 239 default regular let)
	ps1=(
